...
def silver_polig_hellman(base, result, modulo):
    ...
    finish = time.time()
    print(f"SPH: Answer to equation {base}^x = {result} mod {modulo} is {answer}. Time taken {finish-start}")

a = int(input("Input base a : "))
b = int(input("Input result b : "))
p = int(input("Input mod p : "))
silver_polig_hellman(base=a, result=b, modulo=p)
rs, t = bruteforce_log(base=a, result=b, mod=p)
print(f"Bruteforce: Answer to equation {a}^x = {b} mod {p} is {rs}. Time taken {t}")
